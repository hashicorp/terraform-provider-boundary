...
data "boundary_credential_libraries" "foo" {
	depends_on = [boundary_credential_library_vault.example]
		...
		Steps: []resource.TestStep{
			{
				...
				ExpectError: regexp.MustCompile("This field must be a valid credential store id."),
			},
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					resource.TestCheckResourceAttr("data.boundary_credential_libraries.foo", "items.0.%", "11"),
					...
					resource.TestCheckResourceAttr("data.boundary_credential_libraries.foo", "items.0.type", "vault-generic"),
